...
if st.sidebar.button("🔮 Predict Disposition", type="primary"):
    ...
    # Create 76-feature input array with defaults
    input_features = np.zeros((1, 76))
    
    # Set user inputs at appropriate positions
    input_features[0, 0] = orbital_period     # koi_period
    input_features[0, 2] = impact_param       # koi_impact  
    input_features[0, 3] = transit_duration   # koi_duration
    input_features[0, 4] = transit_depth      # koi_depth
    input_features[0, 5] = planetary_radius   # koi_prad
    input_features[0, 6] = equilibrium_temp    # koi_teq
    input_features[0, 7] = insolation_flux     # koi_insol
    
    # Set reasonable defaults for stellar parameters
    input_features[0, 10] = 5500.0  # koi_steff
    input_features[0, 11] = 4.5     # koi_slogg  
    input_features[0, 12] = 1.0     # koi_srad
